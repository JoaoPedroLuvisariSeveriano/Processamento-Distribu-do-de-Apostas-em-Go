...
func IsDuplicateKeyError(err error) bool {
	...
	// pgx v5 usa pgconn.PgError para erros do servidor PostgreSQL
...
// errorAs e um helper para interface assertion que evita import de pgconn.
func errorAs(err error, target interface{}) bool {
	type pgErr interface{ SQLState() string }
	if t, ok := target.(*pgErr); ok {
		type unwrapper interface{ Unwrap() error }
		current := err
		for current != nil {
			if e, ok := current.(pgErr); ok {
				*t = e
				return true
			}
